...
Flask + SQLAlchemy + PostgreSQL (SQLite en local)

Controles de seguridad incluidos:
- Secretos cargados desde variables de entorno (nunca en el código)
- Consultas parametrizadas vía ORM (prevención de inyección SQL)
- Cabeceras de seguridad HTTP (Flask-Talisman)
- Límite de tasa / rate limiting (Flask-Limiter)
...
# La clave secreta y la cadena de conexión SIEMPRE vienen del entorno.
app.config["SECRET_KEY"] = os.environ.get("SECRET_KEY", "cambia-esto-en-produccion")

# Render entrega DATABASE_URL con prefijo postgres://; SQLAlchemy exige postgresql://
...
# ---------------------------------------------------------------------------
# force_https=False porque el proxy de Render ya termina TLS; las cabeceras
# de seguridad (CSP, X-Frame-Options, etc.) sí se aplican.
